...
def create_worker_app(models: GenerationModels, settings: Settings) -> FastAPI:
    ...
    def health() -> dict[str, object]:
        ...
        mounts = {
            "ideogram-4-nf4": ideogram_weights_available(settings.ideogram_weights_path, repository),
            "longcat-image-edit": longcat_weights_available(settings.longcat_edit_weights_path, settings.longcat_edit_revision),
            "longcat-image-edit-turbo": longcat_weights_available(settings.longcat_edit_turbo_weights_path, settings.longcat_edit_turbo_revision),
        }
        return {"ready": cuda and any(mounts.values()), "loaded": models.loaded_model is not None, "device": "cuda" if cuda else "unavailable", "weightsAvailable": all(mounts.values()), "models": {name: {"weightsAvailable": value, "loaded": models.loaded_model == name} for name, value in mounts.items()}, "loadedModel": models.loaded_model}
    ...
    async def edit(file: UploadFile = File(), model: str = "", prompt: str = "", negative_prompt: str = "", seed: int = 0) -> Response:
        try:
            data = await file.read()
            return Response(models({"model": model, "prompt": prompt, "negative_prompt": negative_prompt, "seed": seed, "source_image_bytes": data}), media_type="image/png")
        except Exception as exc:
...
def main() -> None:
    ...
    import uvicorn
    uvicorn.run(create_worker_app(models, settings), host="0.0.0.0", port=int(os.getenv("IMAGE_API_GENERATION_WORKER_PORT", "9003")), log_level=os.getenv("IMAGE_API_LOG_LEVEL", "info").lower())
