...
def ideogram_weights_available(weights_path: Path, repository_id: str) -> bool:
    if not REPOSITORY_ID_PATTERN.fullmatch(repository_id):
        return False
    reference = weights_path / "hub" / f"models--{repository_id.replace('/', '--')}" / "refs" / "main"
    try:
        snapshot = reference.read_text().strip()
    except OSError:
        return False
    root = reference.parent.parent / "snapshots" / snapshot
    return SNAPSHOT_PATTERN.fullmatch(snapshot) is not None and all((root / item).is_file() for item in ("vae/diffusion_pytorch_model.safetensors", "text_encoder/config.json", "tokenizer/tokenizer.json"))
...
def longcat_weights_available(weights_path: Path, revision: str) -> bool:
    try:
        ...
        return False
    return installed == revision and (weights_path / "config.json").is_file() and _weights_available(weights_path / "transformer", "diffusion_pytorch_model.safetensors")
...
class Settings:
    ...
    def from_env(cls) -> Settings:
        return cls(ideogram_weights_path=Path(os.getenv("IMAGE_API_IDEOGRAM_WEIGHTS_PATH", "/models/ideogram-4-nf4")), longcat_edit_weights_path=Path(os.getenv("IMAGE_API_LONGCAT_EDIT_WEIGHTS_PATH", "/models/longcat-image-edit")), longcat_edit_turbo_weights_path=Path(os.getenv("IMAGE_API_LONGCAT_EDIT_TURBO_WEIGHTS_PATH", "/models/longcat-image-edit-turbo")), worker_timeout_seconds=float(os.getenv("IMAGE_API_WORKER_TIMEOUT_SECONDS", "900")), magic_prompt_backend=os.getenv("IMAGE_API_MAGIC_PROMPT_BACKEND") or None, generation_test_mode=os.getenv("IMAGE_API_GENERATION_TEST_MODE", "false").lower() == "true")

    @classmethod
    def for_tests(cls, root: Path, **overrides: Any) -> Settings:
        value = cls(ideogram_weights_path=root / "ideogram", longcat_edit_weights_path=root / "longcat", longcat_edit_turbo_weights_path=root / "longcat-turbo", max_upload_bytes=1_000_000, max_input_pixels=1_000_000, max_decoded_input_bytes=4_000_000, processing_max_upload_bytes=1_000_000, processing_max_encoded_output_bytes=1_000_000, processing_max_input_pixels=1_000_000, processing_max_output_pixels=4_000_000, processing_max_decoded_input_bytes=4_000_000, processing_max_decoded_output_bytes=16_000_000, generation_test_mode=True)
        return replace(value, **overrides)

    def admit_upscale_processing(self, width: int, height: int) -> tuple[int, int]:
        if width * 4 > self.processing_max_native_width or height * 4 > self.processing_max_native_height:
